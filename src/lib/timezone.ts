/**
 * Local wall-clock hour (0-23) for a person's own IANA zone at a given
 * instant (defaulting to now), used to decide "is it their 8pm right now"
 * rather than UTC's - see the cron routes under src/app/api/cron, and (for
 * a past `checkedAt` instead of "now") src/lib/achievements.ts's
 * early-bird/night-owl badges. `null` falls back to UTC (a user whose
 * browser hasn't synced one yet, or a stored value that somehow isn't a
 * real zone Intl recognizes) - both callers still work, just on UTC clock
 * time until a sync happens.
 */
